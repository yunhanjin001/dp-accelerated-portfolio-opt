...
# ─────────────────────────────────────────────────────────
#  DP / LQR Solver  (multi-asset Markowitz)
# ─────────────────────────────────────────────────────────

def solve_lqr_markowitz(T, mu, Sigma, lam, gamma_tc):
    """
    Adjusted LQR solver to match CVXPY objective exactly.
    Objective: minimize sum_{t=0}^{T-1} [ -mu'w_{t+1} + (lam/2)w_{t+1}'Sigma*w_{t+1} + (gamma_tc/2)u_t'u_t ]
    """
    n = len(mu)
    na = n + 1 
...
def execute_lqr_markowitz(T, K_gains, w0):
...
def run_single_comparison(T=30, n=5, lam=2.0, gamma_tc=0.5, seed=42):
    ...
    K  = solve_lqr_markowitz(T, mu, Sigma, lam, gamma_tc)
    w_dp = execute_lqr_markowitz(T, K, w0)
